...
bars = ax.bar(
    p1_model.index,
    p1_model.values,
    color=[COLORS.get(m, "#888") for m in p1_model.index],
)
...
fig.suptitle(
    "Phase 1: Score-Heatmap (Temperature × Repeat Penalty, gemittelt über Top-K)",
    fontsize=13,
)
...
ax.bar(
    hall_by_temp.index.astype(str),
    hall_by_temp.values,
    color=["#22c55e", "#84cc16", "#eab308", "#ef4444"],
)
...
bars = ax.bar(
    p2_model.index,
    p2_model.values,
    color=[COLORS.get(m, "#888") for m in p2_model.index],
)
ax.set_ylabel("Mean Score")
...
for idx, model in enumerate(sorted(df2s["model"].unique())):
    ...
    for i in range(len(pivot.index)):
        for j in range(len(pivot.columns)):
            ax.text(
                j, i, f"{pivot.values[i, j]:.1f}", ha="center", va="center", fontsize=9
            )
...
for idx, model in enumerate(sorted(df2s["model"].unique())):
    ...
    if window > 1:
        ...
        ax.plot(
            ...
            color="red",
            linewidth=1.5,
            label=f"Gleitender Schnitt ({window})",
        )
    ...
    ax.legend()
fig.suptitle(
    "Phase 2: Generierungszeit über den Evaluierungsverlauf (GPU-Stress-Check)",
    fontsize=13,
)
...
pivot3 = df3s.pivot_table(
    values="overall_score", index="question", columns="model", aggfunc="mean"
)
...
models_sorted = (
    df3s.groupby("model")["overall_score"].mean().sort_values(ascending=False).index
)
...
bars = ax.bar(
    err_rates.index,
    err_rates.values,
    color=[COLORS.get(m, "#888") for m in err_rates.index],
)
...
bars = ax.bar(
    k_scores.index.astype(str), k_scores.values, color=["#8B5CF6", "#A78BFA", "#C4B5FD"]
)
ax.set_xlabel("Top-K (Retrieval)")
ax.set_ylabel("Mean Score")
...
fig, ax = plt.subplots(figsize=(14, 8))
q_by_k = qwen14_combined.pivot_table(
    values="overall_score", index="question", columns="top_k", aggfunc="mean"
)
...
ax.bar(
    ...
    bdf["avg_score"],
    0.3,
    label="Durchschnitt (alle Configs)",
    color="#C4B5FD",
)
ax.set_xticks(x)
...
phase_best = {
    ...
    "Phase 5\n(Top-K Fine)": qwen14_combined.groupby("top_k")
    .apply(
        lambda g: g.groupby(["temperature", "repeat_penalty"])["overall_score"]
        .mean()
        .max()
    )
    .max(),
...
ax.plot(
    list(phase_best.keys()),
    list(phase_best.values()),
    "o-",
    color="#8B5CF6",
    linewidth=2,
    markersize=8,
...
for model in df1s["model"].unique():
    best = (
        df1s[df1s["model"] == model]
        .groupby(["temperature", "repeat_penalty", "top_k"])["overall_score"]
        .mean()
        .max()
    )
...
for model in df4s[df4s["provider"] == "local"]["model"].unique():
    ...
    all_models_scores[model]["vs Cloud\n(Phase 4)"] = df4s[
        (df4s["model"] == model) & (df4s["provider"] == "local")
    ]["overall_score"].mean()
...
ax2.plot(
    x, scores, "o-", color="#ef4444", linewidth=2, markersize=8, label="Mean Score"
)
...
for k in [5, 6, 7]:
    ...
    cfg = best.idxmax()
    print(
        f"  K={k}: avg={subset['overall_score'].mean():.2f}, best={best.max():.1f} "
...
q5 = (
    qwen14_combined[qwen14_combined["top_k"] == 5]
    .groupby("question")["overall_score"]
    .mean()
)
q6 = (
    qwen14_combined[qwen14_combined["top_k"] == 6]
    .groupby("question")["overall_score"]
    .mean()
)
q7 = (
    qwen14_combined[qwen14_combined["top_k"] == 7]
    .groupby("question")["overall_score"]
    .mean()
)
...
print(
    f"  Phase 2 (14B Grid):     {len(df2):>5} evals, avg={df2s['overall_score'].mean():.1f}"
)
print(
    f"  Phase 3 (Head-to-Head): {len(df3):>5} evals, avg={df3s['overall_score'].mean():.1f}"
)
print(
    f"  Phase 4 (vs Cloud):     {len(df4):>5} evals, avg={df4s['overall_score'].mean():.1f}"
)
print(
    f"  Phase 5 (Top-K Fine):   {len(df5):>5} evals, avg={df5s['overall_score'].mean():.1f}"
)
