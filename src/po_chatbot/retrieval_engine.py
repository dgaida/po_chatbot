...
class HybridRetrievalEngine:
    def __init__(self):
        ...
        tokenized_corpus = [self._tokenize(doc.get('bm25_text', doc.get('content', ''))) for doc in self.chunks]
    ...
    def _meta_matches_study_program(self, meta: dict, study_program_filter: str | None) -> bool:
    ...
    def _has_auslauf_intent(self, q_lower):
        ...
        if any(w in q_lower for w in ["auslauf", "alte prüfungsordnung", "alten prüfungsordnung",
                                       "alte po", "alten po", "bis wann fertig", "abschließen"]):
            return True
        # PO-Kontext erkennen: 'prüfungsordnung' oder 'po '
        has_po_context = ("prüfungsordnung" in q_lower or "neue po" in q_lower
                          or "neuen po" in q_lower or "alten po" in q_lower
                          or "alte po" in q_lower)
        # Anerkennungs-Keywords (Synonyme für Modulanerkennung)
        recognition_kw = any(w in q_lower for w in ["anerkannt", "anerkennung", "anerkenn",
                                                      "anrechn", "anrechnung",
                                                      "übernommen", "übertrag", "übernehm",
                                                      "äquivalenz", "gleichwertig",
                                                      "gelten", "gültig"])
        # Wechsel-Keywords
        switch_kw = any(w in q_lower for w in ["wechsl", "wechsel", "umstieg", "umsteig"])
    ...
    def _apply_heuristic_boost(self, question, candidates):
        ...
        form_intent = any(w in q_lower for w in ["antrag", "formular", "anmeldung", "bescheinigung", "zulassung"])
        verlaengerung_intent = any(w in q_lower for w in ["verlänger", "abgabefrist", "fristverlängerung"])
        po_intent = any(w in q_lower for w in ["prüfungsordnung", "po4", "po5", "po6", "po 4", "po 5"])
        ...
        for cand in candidates:
            ...
            # Formular/Anmeldung-Erkennung (nur wenn nicht Verlängerung)
                ...
                if any(w in doc_type for w in ["formular", "antrag"]) or \
                   any(w in title for w in ["formular", "antrag", "anmeldung"]):
                    priority -= 100
    ...
    def _inject_keyword_matches(self, question, faculty_filter, study_program_filter, candidates_map):
        ...
        if any(w in q_lower for w in ["anmeldeformular", "anmeldungsformular", "anmeldung"]):
    ...
    def search(self, question, faculty_filter, top_k=5, study_program_filter=None):
        ...
        top_indices = sorted(range(len(bm25_scores)), key=lambda i: bm25_scores[i], reverse=True)[:30]
        
        for idx in top_indices:
            chunk = self.chunks[idx]
            if chunk['metadata'].get('faculty') == faculty_filter and self._meta_matches_study_program(chunk['metadata'], study_program_filter):
        ...
        self._inject_keyword_matches(question, faculty_filter, study_program_filter, candidates_map)
...
if __name__ == "__main__":
    ...
    res = engine.search("Wo finde ich den Antrag auf Zulassung zur Bachelorarbeit?", "F10")
